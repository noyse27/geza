...
export async function POST(req: Request) {
  ...
  if (body.action === 'setup') {
    const existing = await query('SELECT 1 FROM admin_account WHERE id=1');
    if (existing.length)
  ...
  if (!account || !valid || username !== account.username)
    return Response.json({ error: 'Benutzername oder Passwort stimmt nicht.' }, { status: 401 });
  await query("DELETE FROM login_attempts WHERE key='admin'");
  await createSession();
