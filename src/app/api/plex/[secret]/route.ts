...
export async function POST(req: Request, { params }: { params: Promise<{ secret: string }> }) {
  ...
  if (!account || !server)
    return Response.json({ error: 'Plex account and server must be configured' }, { status: 503 });
  try {
    ...
    return Response.json({ error: 'Invalid payload or temporarily unavailable' }, { status: 400 });
  }
}
