...
async function tmdb(m: Raw): Promise<Raw | null> {
  ...
  return {
    ...
    year: Number((d.release_date || d.first_air_date || d.air_date || '').slice(0, 4)) || undefined,
    summary: overview,
    ...
    poster: d.poster_path
      ...
      : d.still_path
        ? `https://image.tmdb.org/t/p/w342${d.still_path}`
        : undefined,
  };
}
...
async function tvdb(m: Raw): Promise<Raw | null> {
  ...
  if (!tvdbSession || tvdbSession.key !== key + pin || tvdbSession.expires < Date.now()) {
    const auth = await json(
      'https://api4.thetvdb.com/v4/login',
      {},
      { apikey: key, ...(pin ? { pin } : {}) },
    );
    tvdbSession = { key: key + pin, token: auth.data.token, expires: Date.now() + 86400000 };
  }
  const headers = { Authorization: `Bearer ${tvdbSession.token}` },
    resource = m.kind === 'episode' ? 'episodes' : 'series';
  const { data: d } = await json(`https://api4.thetvdb.com/v4/${resource}/${m.ids.tvdb}/extended`, headers);
  let translation: Raw = {};
  try {
    translation =
      (await json(`https://api4.thetvdb.com/v4/${resource}/${m.ids.tvdb}/translations/deu`, headers)).data ||
      {};
  } catch {
    /* Original language is a valid fallback. */
  }
  return {
    title: translation.name || d.name,
    original_title: d.name,
    year: Number((d.firstAired || d.aired || '').slice(0, 4)) || undefined,
    summary: translation.overview || d.overview,
    countries: d.originalCountry ? [d.originalCountry] : undefined,
    genres: tags(d.genres),
    actors: d.characters
      ?.filter((x: Raw) => x.peopleType === 'Actor')
