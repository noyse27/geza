...
export async function importTrakt(directory: string) {
  const files = (await readdir(directory)).filter((f) =>
    /^(watched-history-|watched-movies-|watched-shows-|ratings-|comments-|collection-).*\.json$/.test(f),
  );
  ...
  try {
    ...
    for (let i = 0; i < entries.length; i += 1000)
      await client.query(
        `INSERT INTO media(kind,trakt_id,title,year,ids,season,episode)
   ...
   ON CONFLICT(kind,trakt_id) DO UPDATE SET ids=media.ids || excluded.ids`,
        [JSON.stringify(entries.slice(i, i + 1000))],
      );
    const idMap = new Map(
      (await client.query('SELECT id,kind,trakt_id FROM media')).rows.map((r) => [
        `${r.kind}:${r.trakt_id}`,
        r.id,
      ]),
    );
