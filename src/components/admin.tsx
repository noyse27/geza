...
export function AdminControls({ configured }: { configured: Record<string, boolean> }) {
  ...
  return (
    <>
      ...
      >
        <h2>Verbindungen</h2>
        <p className="muted">
          Zugangsdaten werden verschlüsselt gespeichert. Leere Felder behalten den bisherigen Wert.
        </p>
          ...
          {[
            ...
            <label key={key}>
              ...
              <input
                ...
                autoComplete="off"
                placeholder={configured[key] ? 'Gespeichert — zum Ändern neu eingeben' : ''}
              />
            </label>
          ))}
        </div>
