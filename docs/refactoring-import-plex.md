...
### Ergänzung vom 26. September: vollständiger Serienkatalog und gemeinsame Sichtungen

- Der bisherige Staffelaufbau aus vorhandenen Episoden genügt nicht für manuell hinzugefügte Serien oder gemischte Prime-/Plex-Sichtungen. Ein eigener TMDB-Katalogabgleich ergänzt Staffeln und Episoden unabhängig von Plex. Provider-ID und Serien-/Staffel-/Episodennummer dienen der Zuordnung; manuell gesperrte Zuordnungen bleiben erhalten. Fehlgeschlagene/mehrdeutige Katalogantworten werden nicht teilweise veröffentlicht.
- Migration 021 merkt bestehende Serien außerhalb der Rumpelkammer zur Nachbearbeitung vor. Der Worker führt sie nach dem Update mit konfiguriertem TMDB aus; die Migration selbst benötigt keinen Netzwerkzugriff. Fehlende Zuordnungen bleiben im Jobprotokoll sichtbar. Neue Bucketlistenserien und geöffnete Serien-/Staffeldetails planen den Katalogabgleich ebenfalls ein.
- Bestehende, vor der Migration angelegte manuelle Serien-/Staffelsichtungen mit Datum werden einmalig auf noch unbelegte Folgen übertragen. Bereits erfasste Plex-/Trakt-/manuelle Sichtungen und ihre Daten bleiben erhalten. Folgen mit einem bekannten späteren Erscheinungsdatum werden ausgeschlossen; bei unbekanntem Erscheinungsdatum werden die bekannten Folgen berücksichtigt. Später ergänzte Folgen erhalten keine fortlaufende automatische Gesehenmarkierung.
- Bei neuen manuellen Serien-/Staffelsichtungen ist „Datum auch auf Folgen anwenden“ vorausgewählt und abwählbar. Der Katalog wird mit TMDB vor dem Speichern ergänzt; ein Abruffehler verhindert das Speichern der Gesamtsichtung. Ohne TMDB gilt die Aktion ausdrücklich nur für lokal bekannte Folgen. Vorhandene Ereignisse können über „Datum auf fehlende Sichtungen der Folgen anwenden“ nachträglich angewendet werden.
- Ergänzte Sichtungen sind über ihre source_id mit dem ursprünglichen Ereignis verbunden. Dessen Datumsänderung oder Löschung gilt auch für diese Ergänzungen. Einzeln korrigierte Folgen werden davon gelöst. Bewertungen und Reviews werden niemals vererbt.
- Jede Folge zeigt im Adminbereich ihren Sichtungsstatus und einen direkten Editor zum Nachtragen. Ein reiner Katalogeintrag bedeutet weder „gesehen“ noch „in Plex vorhanden“.

Regression: `node --import tsx scripts/series-catalog-check.ts` prüft Migration/Nachbearbeitung, reine Plex-Teilsichtungen, unveränderte historische Daten, zukünftige Folgen, Wiederholbarkeit, spätere Katalogergänzungen, Staffelumfang, Abruffehler und gesperrte Zuordnungen. Der HTTP-Test prüft gemeinsame Sichtungen einschließlich Bearbeiten, Löschen und individueller Korrekturen. TMDB-Endpunkte: [Seriendetails](https://developer.themoviedb.org/reference/tv-series-details), [Staffeldetails](https://developer.themoviedb.org/reference/tv-season-details).
