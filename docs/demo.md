...
`demo.geza` enthält ausschließlich erfundene Daten, keine Inhalte aus der privaten Sicherung. Sie wird automatisch bei Installation und Reset eingespielt. Erneut erzeugen: `node --import tsx scripts/generate-demo.ts`. Die Datei kann auch in einer leeren normalen Installation über die vorhandene Wiederherstellung importiert werden. Der öffentliche Wiederherstellungsschlüssel besteht aus **64 Nullen**; beim Import ohne Konten kann ein eigenes sicheres Passwort gewählt werden. **Die Datei aktiviert selbst keinen Demomodus.** Das bekannte Demo-Konto niemals als produktives Konto verwenden.
